      ...
      if (data.includes('Failed to launch the process') && data.includes('ENOENT')) {
        ...
        resolve({
          error:
            ...
            messages.pgettext('split-tunneling-view', 'Program not found.'),
        });
      }
    });
    proc.once('exit', (code) => {
      ...
      if (code === 1) {
        resolve({
          error:
            ...
            messages.pgettext('split-tunneling-view', 'Application exited during startup.'),
        });
      } else {
        resolve({ success: true });
      }
    ...
    if (entry.exec !== undefined) {
      ...
      throw new Error(
        ...
        messages.pgettext(
          ...
          'Desktop file lacks exec property in the Desktop entry group.',
        ),
      );
    }
  } else {
